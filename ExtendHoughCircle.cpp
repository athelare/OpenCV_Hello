...
UINT CheckCircleOverlap( Mat& img, Point center, int radius, UINT &overlap_num, int shift = 0 )
{
...
EXTENDALGORITHM_API void ExtendCV::FindCircles( InputArray _image, vector<circle_found>& _circles,float dp, int min_dist,
	...
	float minScore, InputArray _contour_image )
{
	...
	CvMat c_image = image;
	Mat contour_image = _contour_image.getMat();
	...
	drawContours(contour_image,contours,-1,Scalar(255),overlap_check_brush);
...
// 	Mat draw_mat=Mat(contour_image.rows,contour_image.cols,CV_8UC1);
	...
	for (int i=0,j=0;i<vector_size;i++)
	{
		NonZeroCount_circle=CheckCircleOverlap(contour_image,
			Point(circles_all[i][0],circles_all[i][1]),circles_all[i][2],NonZeroCount_overlap);
